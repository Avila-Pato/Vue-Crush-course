...
// Define el estado reactivo para el formulario
const form = reactive({
  type: 'Full-Time',
  title: '',
  description: '',
...
const handleSubmit = async () => {
  ...
  try {
    const response = await axios.post('api/jobs', newJob) // Envía los datos a la API
    toast.success('Job added successfully')
    router.push(`/jobs/${response.data.id}`) // Redirige a la página del nuevo trabajo
  } catch (error) {
    console.error('error fetching job', error)
    toast.error('jobs was not added')
  }
}
</script>
